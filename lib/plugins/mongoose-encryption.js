 ...
 * @param      {'store'|'require'|'disable'}    [options.fieldsList='store']
...
var mongooseEncryption = function(schema, options) {
    var encryptedFields, excludedFields, authenticatedFields, encryptionKey, signingKey, fieldsList;
    ...
    /** Decryption Options */
    fieldsList = options.requireFieldsList || 'store'


    /** Encryption Options */

    if (options.encryptedFields) {
        encryptedFields = _.difference(options.encryptedFields, ['_ct'], fieldsList === 'disable' ? undefined : ['_fields']);
    } else {
        excludedFields = _.union(['_id', '_ct'], options.excludeFromEncryption, fieldsList === 'disable' ? undefined : ['_fields']);
    ...
    var baselineAuthenticateFields = _.union(['_id', '_ct'], fieldsList === 'disable' ? undefined : ['_fields']);
    ...
    if (fieldsList !== 'disable' && !schema.paths._fields) { // encrypted fields list
        schema.add({
            _fields: {
                type: String
            }
    ...
    schema.methods.encrypt = function(cb) {
        ...
        crypto.randomBytes(IV_LENGTH, function(err, iv) {
            ...
            if (fieldsList !== 'disable') {
                that._fields = encryptedFields.join(',');
            }
    ...
    schema.methods.decryptSync = function() {
        var that = this;
        var ct, fields, ctWithIV, decipher, iv, idString, decryptedObject, decryptedObjectJSON, decipheredVal;
        if (this._ct) {
            ...
            if (fieldsList === 'require') {
                fields = this._fields ? this._fields.split(',') : undefined
                if (!fields) {
                    throw new Error('Fields list is required, but not found during decrypt of ' + idString)
                }
            } else if (fieldsList === 'disable') {
                fields = encryptedFields
            } else {
                fields = this._fields ? this._fields.split(',') : encryptedFields
            }
