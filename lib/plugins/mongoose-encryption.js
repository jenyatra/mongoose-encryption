...
var mongooseEncryption = function(schema, options) {
    ...
    schema.methods.decryptCt = function(_ct) {
        var ct, cf, ctWithIV, decipher, iv, idString, decryptedObject, decryptedObjectJSON;
        ...
        return { decryptedObject, cf };
    };

    schema.methods.decryptTo = function(data) {
        var decryptResult, cf, decryptedObject, decipheredVal;

        decryptResult = this.decryptCt(data._ct)

        decryptedObject = decryptResult.decryptedObject
        cf = decryptResult.cf
