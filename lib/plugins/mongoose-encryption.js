...
var mongooseEncryption = function(schema, options) {
    ...
    if (cfMode !== 'disable' && _.contains(encryptedFields, '_cf')) {
        throw new Error('_cf field cannot be encrypted when cf is enabled. Exclude it from encryption or use cfMode: \'disable\'');
    }
    ...
    schema.methods.updateCf = function (cb) {
    ...
    schema.methods.decryptTo = function(data) {
        var ct, cf, ctWithIV, decipher, iv, decryptedObject, decryptedObjectJSON, decipheredVal;
        ...
        try {
            ...
            throw new Error('Error during decrypt of ' + getIdString(this) + ': ' + error);
        }
